document.addEventListener('DOMContentLoaded', () => {
    ...
    window.updateReceiveGrandTotal = () => {
        ...
        } catch (e) { Logger.error('Error in updateReceiveGrandTotal', e); }
    };

    window.updateTransferGrandTotal = () => {
        try {
            let grandTotalQty = 0;
            ...
            if (el) el.textContent = grandTotalQty.toFixed(2);
        } catch (e) { Logger.error('Error in updateTransferGrandTotal', e); }
    };

    window.updateIssueGrandTotal = () => {
        try {
            let grandTotalQty = 0;
            (state.currentIssueList || []).forEach(item => {
                grandTotalQty += (parseFloat(item.quantity) || 0);
            });
            const el = document.getElementById('issue-grand-total');
            if (el) el.textContent = grandTotalQty.toFixed(2);
        } catch (e) { Logger.error('Error in updateIssueGrandTotal', e); }
    };

    window.updatePOGrandTotal = () => {
        try {
            let grandTotal = 0;
            ...
            if (el) el.textContent = `${grandTotal.toFixed(2)} EGP`;
        } catch (e) { Logger.error('Error in updatePOGrandTotal', e); }
    };

    window.updatePOEditGrandTotal = () => {
        try {
            let grandTotal = 0;
            ...
            if (el) el.textContent = `${grandTotal.toFixed(2)} EGP`;
        } catch (e) { Logger.error('Error in updatePOEditGrandTotal', e); }
    };

    window.updateReturnGrandTotal = () => {
        try {
            let grandTotal = 0;
            ...
            if (el) el.textContent = `${grandTotal.toFixed(2)} EGP`;
        } catch (e) { Logger.error('Error in updateReturnGrandTotal', e); }
    };
    ...
    window.reloadDataAndRefreshUI = async () => {
        ...
        try {
            ...
            Object.keys(data).forEach(key => {
                if (key === 'user') return;
                // Ensure companySettings is always an object
    ...
    function attachEventListeners() {
        ...
        // --- Event Listeners for Confirm Receipt Modal ---
    ...
    function attachFormListeners() {
        if (document.getElementById('form-add-item')) {
            document.getElementById('form-add-item').addEventListener('submit', async e => {
                ...
                const result = await postData('addItem', data, btn, 'Saving...');
                if (result) {
        ...
        if (document.getElementById('form-add-supplier')) {
            document.getElementById('form-add-supplier').addEventListener('submit', async e => {
                ...
                const result = await postData('addSupplier', data, btn, 'Saving...');
                if (result) {
                    showToast(_t('add_success_toast', { type: _t('supplier') }), 'success');
                    e.target.reset();
                    reloadDataAndRefreshUI();
                }
            });
        }
        if (document.getElementById('form-add-branch')) {
            document.getElementById('form-add-branch').addEventListener('submit', async e => {
                ...
                const result = await postData('addBranch', data, btn, 'Saving...');
                if (result) {
                    showToast(_t('add_success_toast', { type: _t('branch') }), 'success');
                    e.target.reset();
                    reloadDataAndRefreshUI();
                }
            });
        ...
        if (document.getElementById('form-add-section')) {
            document.getElementById('form-add-section').addEventListener('submit', async e => {
                ...
                const result = await postData('addSection', data, btn, 'Saving...');
                if (result) {
                    showToast(_t('add_success_toast', { type: _t('section') }), 'success');
                    e.target.reset();
                    reloadDataAndRefreshUI();
                }
            });
        ...
        if (document.getElementById('btn-submit-receive-batch')) document.getElementById('btn-submit-receive-batch').addEventListener('click', async (e) => {
            ...
            const supplierCode = document.getElementById('receive-supplier').value,
                ...
                poId = document.getElementById('receive-po-select').value;
            ...
            if (!userCan('opReceiveWithoutPO') && !poId) {
                showToast(_t('select_po_first_toast'), 'error');
                return;
            }
            ...
            const payload = {
                ...
                items: state.currentReceiveList.map(i => ({ ...i, type: 'receive' })),
                notes
            };
            await handleTransactionSubmit(payload, btn);
        });

        if (document.getElementById('btn-submit-transfer-batch')) document.getElementById('btn-submit-transfer-batch').addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            let fromBranchCode = document.getElementById('transfer-from-branch').value,
                toBranchCode = document.getElementById('transfer-to-branch').value;
            const notes = document.getElementById('transfer-notes').value,
                ref = document.getElementById('transfer-ref').value;
            if (userCan('viewAllBranches') && !state.currentUser.AssignedBranchCode) {
            ...
            if (!fromBranchCode || !toBranchCode || fromBranchCode === toBranchCode || state.currentTransferList.length === 0) {
                showToast('Please select valid branches and add at least one item.', 'error');
                return;
            }
            const payload = {
                type: 'transfer_out',
        ...
        if (document.getElementById('btn-submit-issue-batch')) document.getElementById('btn-submit-issue-batch').addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            let fromBranchCode = document.getElementById('issue-from-branch').value,
                sectionCode = document.getElementById('issue-to-section').value;
            ...
            if (!fromBranchCode || !sectionCode || !ref || state.currentIssueList.length === 0) {
                showToast('Please fill all issue details and select at least one item.', 'error');
                return;
            }
            const payload = {
                type: 'issue',
        ...
        if (document.getElementById('btn-submit-po')) document.getElementById('btn-submit-po').addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            const supplierCode = document.getElementById('po-supplier').value,
                poId = document.getElementById('po-ref').value,
        ...
        if (document.getElementById('btn-submit-return')) document.getElementById('btn-submit-return').addEventListener('click', async (e) => {
            const btn = e.currentTarget;
        ...
        if (document.getElementById('btn-submit-request')) document.getElementById('btn-submit-request').addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            let fromSection = state.currentUser.AssignedSectionCode,
                toBranch = state.currentUser.AssignedBranchCode;
            ...
            if (!fromSection || !toBranch) {
                showToast('Your user is not assigned a branch/section to make requests. Please contact an admin.', 'error');
                return;
            }
            const payload = {
            ...
            const result = await postData('addItemRequest', payload, btn, 'Submitting...');
            if (result) {
        ...
        if (document.getElementById('btn-submit-adjustment')) document.getElementById('btn-submit-adjustment').addEventListener('click', async (e) => {
            ...
            if (adjustmentItems.length === 0) {
                showToast('No adjustments needed.', 'info');
                return;
            }
            const payload = {
        ...
        if (document.getElementById('form-record-payment')) document.getElementById('form-record-payment').addEventListener('submit', async e => {
            e.preventDefault();
            const btn = e.target.querySelector('button[type="submit"]');
            ...
            if (!supplierCode || state.invoiceModalSelections.size === 0) {
                showToast('Please select supplier and invoices.', 'error');
                return;
            }
        ...
        if (document.getElementById('form-financial-adjustment')) document.getElementById('form-financial-adjustment').addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = e.target.querySelector('button[type="submit"]');
            ...
            const payload = { supplierCode: supplierCode, balance: balance, ref: `OB-${supplierCode}` };
        ...
        if (document.getElementById('form-company-settings')) document.getElementById('form-company-settings').addEventListener('submit', async e => {
            e.preventDefault();
            const btn = e.target.querySelector('button[type="submit"]');
    ...
    function handleMainContentClicks(e) {
        ...
        if (btn.id === 'btn-add-new-role') {
            const roleName = prompt(_t('add_role_prompt'));
            if (roleName) {
                postData('addRole', { RoleName: roleName }, btn, 'Adding...').then(res => res && reloadDataAndRefreshUI());
            }
        }
        ...
        if (btn.classList.contains('btn-cancel-transfer')) {
            const batchId = btn.dataset.batchId;
            if (confirm(`Cancel transfer ${batchId}?`)) {
                postData('cancelTransfer', { batchId }, btn, 'Cancelling...').then(res => res && reloadDataAndRefreshUI());
            }
        }
        ...
        if (btn.classList.contains('btn-reject-request')) {
            if (confirm('Reject request?')) postData('rejectItemRequest', { requestId: btn.dataset.id }, btn, 'Rejecting...').then(res => res && reloadDataAndRefreshUI());
        }
        ...
        if (btn.classList.contains('btn-approve-financial') || btn.classList.contains('btn-reject-financial')) {
            const id = btn.dataset.id,
                ...
                action = btn.classList.contains('btn-approve-financial') ? 'approveFinancial' : 'rejectFinancial';
            if (confirm(`Confirm ${action.replace('Financial', '')} for ${type}?`)) {
                postData(action, { id, type }, btn, 'Processing...').then(res => {
                    if (res) {
