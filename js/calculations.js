...
export const calculateSupplierFinancials = () => {
    ...
    Object.values(financials).forEach(s => {
        ...
        Object.values(s.invoices).forEach(inv => { 
            ...
            if (Math.abs(inv.balance) < 0.01) { inv.status = 'Paid'; } 
    ...
    financials.allInvoices = {}; 
    Object.values(financials).forEach(s => { Object.assign(financials.allInvoices, s.invoices); }); 
    return financials;
};

export const calculateHistoricalCosts = () => {
    const costSnapshots = {}; 
    const stock = {}; 
    (state.branches || []).forEach(branch => { stock[branch.branchCode] = {}; });
    const sortedTransactions = [...(state.transactions || [])].filter(t => t.type).sort((a, b) => new Date(a.date) - new Date(b.date));
